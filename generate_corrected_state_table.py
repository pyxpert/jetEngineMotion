...
def generate_motor_positions_and_states_corrected():
    """
    生成机器人运动过程中的详细状态表（使用六位数字状态码并正确实现磁铁抬起机制）
    状态码格式：ω1ω2ω3vm1m2
...
# 生成状态列表
states_list, base_angle, lift_angle = generate_motor_positions_and_states_corrected()

# 写入详细状态表到文件
with open('robot_detailed_state_table_corrected.txt', 'w', encoding='utf-8') as f:
    f.write('"""机器人运动状态详细记录表（最终版 - 正确的磁铁抬起机制）\n')
    f.write('时间：2026-01-28\n')
    f.write('任务：从初始位置(前足0,40；后足0,0)爬行至目标位置(前足0,1000)\n')
    f.write('基于优化轨迹（255步）\n')
    f.write('使用六位数字状态码：ω1ω2ω3vm1m2\n')
    f.write('正确实现：通过中间旋转电机动作抬起磁铁\n')
    f.write('"""\n\n')
