"""
机器人运动状态详细记录表（最终完全修正版 - 正确的电机动作逻辑和时间参数）
时间：2026-01-29
...
def generate_completely_corrected_motor_positions_and_states_with_time():
    """
    ...
    lower_angle = -45.0   # 放下角度（吸附准备状态）- 反转角度
        ...
        if step_count % 2 == 0:  # 移动前足
            ...
            if min(robot_params['d1'] + robot_params['d2'], new_spacing) <= robot_params['d1'] + robot_params['d2'] + robot_params['s_max']:
                ...
                linear_extend_state = {
                    ...
                    'rear_position': current_rear.copy(),
                    'center_motor_angle': lift_angle,  # 保持抬起前磁铁（正转）
                ...
                prep_attach_front_state = {
                    ...
                    'front_position': new_front_pos,
                    'rear_position': current_rear.copy(),
                    'center_motor_angle': lower_angle,  # 前磁铁准备放下（通过旋转电机反转动作）
                    'linear_motor_pos': linear_motor_delta,
                    'front_magnet_attach_pos': new_front_pos,  # 即将吸附的位置
                    'rear_magnet_attach_pos': current_rear.copy(),
                    'duration_ms': int(abs(lower_angle - lift_angle) / motor_speeds['center_rotation_speed'] * 1000)  # 旋转电机转动时间
                }
                states_list.append(prep_attach_front_state.copy())
                
                # 最终放下前足吸附（回到基础角度）
                lower_front_to_attach_state = {
                    'step': step_count + 1,
                    'state_code': '000001',  # 中间旋转电机继续反转使前足完全放下准备吸附，前磁铁仍脱附，后磁铁吸附
                    'front_magnet_status': '脱附',
                    'rear_magnet_status': '吸附',
                    'front_position': new_front_pos,
                    'rear_position': current_rear.copy(),
                    'center_motor_angle': base_angle,  # 前磁铁完全放下（通过旋转电机反转到基础角度）
                    'linear_motor_pos': linear_motor_delta,
                    'front_magnet_attach_pos': new_front_pos,  # 即将吸附的位置
                    'rear_magnet_attach_pos': current_rear.copy(),
                    'duration_ms': int(abs(base_angle - lower_angle) / motor_speeds['center_rotation_speed'] * 1000)  # 旋转电机转动时间
                }
                states_list.append(lower_front_to_attach_state.copy())
                ...
                if min(robot_params['d1'] + robot_params['d2'], new_spacing) <= robot_params['d1'] + robot_params['d2'] + robot_params['s_max']:
                    ...
                    linear_contract_state = {
                        ...
                        'front_position': current_front.copy(),
                        'rear_position': new_rear_pos,
                        'center_motor_angle': lift_angle,  # 保持抬起后磁铁（正转）
                        'linear_motor_pos': linear_motor_delta,
                        'front_magnet_attach_pos': current_front.copy(),
                        'rear_magnet_attach_pos': current_rear.copy(),
                        'duration_ms': int(abs(linear_motor_delta - prev_linear_motor_pos) / motor_speeds['linear_speed'] * 1000) if abs(linear_motor_delta - prev_linear_motor_pos) > 0.1 else 50  # 直线电机运行时间，最小50ms
                    ...
                    prep_attach_rear_state = {
                        ...
                        'front_position': current_front.copy(),
                        'rear_position': new_rear_pos,
                        'center_motor_angle': lower_angle,  # 后磁铁准备放下（通过旋转电机反转动作）
                        'linear_motor_pos': linear_motor_delta,
                        'front_magnet_attach_pos': current_front.copy(),
                    ...
                    # 最终放下后足吸附（回到基础角度）
                    lower_rear_to_attach_state = {
                        'step': step_count + 1,
                        'state_code': '000010',  # 中间旋转电机继续反转使后足完全放下准备吸附，前磁铁吸附，后磁铁仍脱附
                        'front_magnet_status': '吸附',
                        'rear_magnet_status': '脱附',
                        'front_position': current_front.copy(),
                        'rear_position': new_rear_pos,
                        'center_motor_angle': base_angle,  # 后磁铁完全放下（通过旋转电机反转到基础角度）
                        'linear_motor_pos': linear_motor_delta,
                        'front_magnet_attach_pos': current_front.copy(),
                        'rear_magnet_attach_pos': new_rear_pos,  # 即将吸附的位置
                        'duration_ms': int(abs(base_angle - lower_angle) / motor_speeds['center_rotation_speed'] * 1000)  # 旋转电机转动时间
                    }
                    states_list.append(lower_rear_to_attach_state.copy())
    ...
    return states_list, base_angle, lift_angle, lower_angle

# 生成状态列表
states_list, base_angle, lift_angle, lower_angle = generate_completely_corrected_motor_positions_and_states_with_time()

# 写入详细状态表到文件
with open('robot_detailed_state_table_completely_corrected_with_time_and_speed.txt', 'w', encoding='utf-8') as f:
    f.write('"""机器人运动状态详细记录表（最终完全修正版 - 正确的电机动作逻辑和时间参数）\n')
...
print("机器人运动状态详细记录表（最终完全修正版 - 正确的电机动作逻辑和时间参数）已生成完成")
