...
def generate_totally_corrected_motor_positions_and_states_with_time():
    """
    ...
    # 记录当前中间电机角度，用于连续状态之间的角度计算
    current_center_angle = base_angle
    ...
    while current_front[1] < target_front_z and step_count < max_steps:
        ...
        if step_count % 2 == 0:  # 移动前足
            # 状态变化：后足保持吸附，前足准备脱离
            # 抬起前足：中间旋转电机正转到lift_angle
            current_center_angle = base_angle  # 开始时在基础角度
            target_angle = lift_angle  # 目标角度
            ...
            lift_front_state = {
                ...
                'front_position': current_front.copy(),
                'rear_position': current_rear.copy(),
                'center_motor_angle': target_angle,  # 抬起前磁铁到45度
                ...
                'duration_ms': int(abs(target_angle - current_center_angle) / motor_speeds['center_rotation_speed'] * 1000),  # 旋转电机转动时间
            }
            states_list.append(lift_front_state.copy())
            current_center_angle = target_angle  # 更新当前角度
            ...
            if min(robot_params['d1'] + robot_params['d2'], new_spacing) <= robot_params['d1'] + robot_params['d2'] + robot_params['s_max']:
                ...
                # 此时中间电机应该停止，保持抬起前足的角度，所以状态码是000101而不是001101
                linear_extend_state = {
                    'step': step_count + 1,
                    'state_code': '000101',  # 中间旋转电机停止保持抬起前足，直线电机正转伸展，前磁铁脱附，后磁铁吸附
                    'front_magnet_status': '脱附',
                    'rear_magnet_status': '吸附',
                    'front_position': new_front_pos,
                    'rear_position': current_rear.copy(),
                    'center_motor_angle': current_center_angle,  # 保持当前角度（中间电机停止）
                ...
                # 放下前足准备吸附：中间旋转电机反转放下前足
                target_angle = base_angle  # 目标角度是基础角度0度
                prep_lower_front_state = {
                    ...
                    'front_position': new_front_pos,
                    'rear_position': current_rear.copy(),
                    'center_motor_angle': target_angle,  # 前磁铁放下（从45°反转到0°）
                    'linear_motor_pos': linear_motor_delta,
                ...
                attach_front_state = {
                    ...
                    'front_position': new_front_pos,
                    'rear_position': current_rear.copy(),
                    'center_motor_angle': current_center_angle,  # 前磁铁吸附时保持基础角度
                    'linear_motor_pos': linear_motor_delta,
                ...
                if min(robot_params['d1'] + robot_params['d2'], new_spacing) <= robot_params['d1'] + robot_params['d2'] + robot_params['s_max']:
                    ...
                    # 抬起后足：中间旋转电机正转到lift_angle
                    current_center_angle = base_angle  # 重置到基础角度
                    target_angle = lift_angle  # 目标角度
                    ...
                    lift_rear_state = {
                        ...
                        'center_motor_angle': target_angle,  # 抬起后磁铁到45度
                    ...
                    linear_contract_state = {
                        'step': step_count + 1,
                        'state_code': '000210',  # 中间旋转电机停止保持抬起后足，直线电机反转收缩，前磁铁吸附，后磁铁脱附
                        'front_magnet_status': '吸附',
                        'rear_magnet_status': '脱附',
                        'front_position': current_front.copy(),
                        'rear_position': new_rear_pos,
                    ...
                    prep_lower_rear_state = {
                        ...
                        'front_position': current_front.copy(),
                        'rear_position': new_rear_pos,
                        'center_motor_angle': target_angle,  # 后磁铁放下（从45°反转到0°）
                        'linear_motor_pos': linear_motor_delta,
                        'front_magnet_attach_pos': current_front.copy(),
                    ...
                    attach_rear_state = {
                        ...
                        'front_position': current_front.copy(),
                        'rear_position': new_rear_pos,
                        'center_motor_angle': current_center_angle,  # 后磁铁吸附时保持基础角度
                        'linear_motor_pos': linear_motor_delta,
                        'front_magnet_attach_pos': current_front.copy(),
...
# 生成状态列表
states_list, base_angle, lift_angle = generate_totally_corrected_motor_positions_and_states_with_time()

# 写入详细状态表到文件
with open('robot_detailed_state_table_totally_corrected_with_time_and_speed.txt', 'w', encoding='utf-8') as f:
