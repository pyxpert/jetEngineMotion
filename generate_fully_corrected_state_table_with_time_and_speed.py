"""
机器人运动状态详细记录表（最终修正版 - 正确的电机动作逻辑和时间参数）
时间：2026-01-29
...
def generate_fully_corrected_motor_positions_and_states_with_time():
    """
    ...
    正确实现磁铁抬起/放下：
    - 脱附时：旋转电机转动一定角度抬起对应磁铁（状态1或2）
    - 吸附前：旋转电机转回原位放下磁铁（状态0）
    - 修正：只有在需要抬起磁铁时才使用非零状态，其他时候保持状态一致
    """
    # 机器人参数
    robot_params = {
        'd1': 20,      # 前足到中间电机的水平距离
    ...
    lift_angle_positive = 45.0     # 抬起前足角度（脱附状态）
    lift_angle_negative = -45.0    # 抬起后足角度（脱附状态）
    ...
    while current_front[1] < target_front_z and step_count < max_steps:
        ...
        if step_count % 2 == 0:  # 移动前足
            ...
            detach_front_state = {
                ...
                'front_position': current_front.copy(),
                'rear_position': current_rear.copy(),
                'center_motor_angle': lift_angle_positive,  # 抬起前磁铁（通过旋转电机动作）
                ...
                'duration_ms': int(abs(lift_angle_positive - base_angle) / motor_speeds['center_rotation_speed'] * 1000)  # 旋转电机转动时间
            }
            ...
            if min(robot_params['d1'] + robot_params['d2'], new_spacing) <= robot_params['d1'] + robot_params['d2'] + robot_params['s_max']:
                ...
                # 修正：这里状态码应该是001101，而不是001201
                # 第三位ω3仍为1表示中间旋转电机保持抬起前足状态
                # 第四位v=1表示直线电机正转（伸展）
                linear_extend_state = {
                    'step': step_count + 1,
                    'state_code': '001101',  # 中间旋转电机保持抬起前足，直线电机正转伸展，前磁铁脱附，后磁铁吸附
                    'front_magnet_status': '脱附',
                    'rear_magnet_status': '吸附',
                    'front_position': new_front_pos,
                    'rear_position': current_rear.copy(),
                    'center_motor_angle': lift_angle_positive,  # 保持抬起前磁铁
                ...
                # 前足到达新位置，准备放下吸附（中间旋转电机调整角度放下磁铁）
                prep_attach_front_state = {
                    'step': step_count + 1,
                    'state_code': '000001',  # 中间旋转电机反转放下前足准备吸附，直线电机停止，前磁铁仍脱附，后磁铁吸附
                    'front_magnet_status': '脱附',
                    'rear_magnet_status': '吸附',
                    'front_position': new_front_pos,
                    'rear_position': current_rear.copy(),
                    ...
                    'rear_magnet_attach_pos': current_rear.copy(),
                    'duration_ms': int(abs(base_angle - lift_angle_positive) / motor_speeds['center_rotation_speed'] * 1000)  # 旋转电机转动时间
                }
                states_list.append(prep_attach_front_state.copy())
                ...
                if min(robot_params['d1'] + robot_params['d2'], new_spacing) <= robot_params['d1'] + robot_params['d2'] + robot_params['s_max']:
                    ...
                    detach_rear_state = {
                        ...
                        'center_motor_angle': lift_angle_negative,  # 抬起后磁铁（通过旋转电机反向动作）
                        'linear_motor_pos': abs(current_front[1] - new_rear_pos[1]) - (robot_params['d1'] + robot_params['d2']),
                        'front_magnet_attach_pos': current_front.copy(),
                        'rear_magnet_attach_pos': current_rear.copy(),
                        'duration_ms': int(abs(lift_angle_negative - base_angle) / motor_speeds['center_rotation_speed'] * 1000)  # 旋转电机转动时间
                    }
                    ...
                    # 修正：这里状态码应该是002210
                    # 第三位ω3为2表示中间旋转电机保持抬起后足状态
                    # 第四位v=2表示直线电机反转（收缩）
                    linear_contract_state = {
                        'step': step_count + 1,
                        'state_code': '002210',  # 中间旋转电机保持抬起后足，直线电机反转收缩，前磁铁吸附，后磁铁脱附
                        'front_magnet_status': '吸附',
                        'rear_magnet_status': '脱附',
                        'front_position': current_front.copy(),
                        'rear_position': new_rear_pos,
                        'center_motor_angle': lift_angle_negative,  # 保持抬起后磁铁
                        'linear_motor_pos': linear_motor_delta,
                        'front_magnet_attach_pos': current_front.copy(),
                        'rear_magnet_attach_pos': current_rear.copy(),
                        'duration_ms': int(abs(linear_motor_delta - prev_linear_motor_pos) / motor_speeds['linear_speed'] * 1000) if abs(linear_motor_delta - prev_linear_motor_pos) > 0.1 else 50  # 直线电机运行时间，最小50ms
                    ...
                    # 后足到达新位置，准备放下吸附（中间旋转电机调整角度放下磁铁）
                    prep_attach_rear_state = {
                        'step': step_count + 1,
                        'state_code': '000010',  # 中间旋转电机正转放下后足准备吸附，直线电机停止，前磁铁吸附，后磁铁仍脱附
                        'front_magnet_status': '吸附',
                        'rear_magnet_status': '脱附',
                        'front_position': current_front.copy(),
                        'rear_position': new_rear_pos,
                        ...
                        'duration_ms': int(abs(base_angle - lift_angle_negative) / motor_speeds['center_rotation_speed'] * 1000)  # 旋转电机转动时间
                    }
                    states_list.append(prep_attach_rear_state.copy())
            ...
            # 状态变化：前足保持吸附，后足准备脱离（中间旋转电机反转抬起后足）
            detach_rear_state = {
                'step': step_count + 1,
                'state_code': '002010',  # 中间旋转电机反转抬起后足，后磁铁脱附，前磁铁仍吸附
                'front_magnet_status': '吸附',
    ...
    return states_list, base_angle, lift_angle_positive, lift_angle_negative

# 生成状态列表
states_list, base_angle, lift_angle_positive, lift_angle_negative = generate_fully_corrected_motor_positions_and_states_with_time()

# 写入详细状态表到文件
with open('robot_detailed_state_table_fully_corrected_with_time_and_speed.txt', 'w', encoding='utf-8') as f:
    f.write('"""机器人运动状态详细记录表（最终修正版 - 正确的电机动作逻辑和时间参数）\n')
...
print("机器人运动状态详细记录表（最终修正版 - 正确的电机动作逻辑和时间参数）已生成完成")
...
print("\n磁铁抬起/放下机制：")
print("- 脱附时：旋转电机转动一定角度抬起对应磁铁 (例如从0°到±45°)")
print("- 吸附前：旋转电机转回原位放下磁铁 (从±45°回到0°)")
print("\n表格格式说明：")
