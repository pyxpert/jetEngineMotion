"""
机器人运动状态详细记录表（最终修正版 - 正确的中心电机角度控制）
时间：2026-01-28
任务：从初始位置(前足0,40；后足0,0)爬行至目标位置(前足0,1000)
基于优化轨迹（255步）
使用六位数字状态码：ω1ω2ω3vm1m2
正确实现：中心电机角度范围为0-90度，正转为内折，反转为外翻
"""

import numpy as np

def generate_motor_positions_and_states_final():
    """
    生成机器人运动过程中的详细状态表（使用六位数字状态码并正确实现中心电机角度控制）
    状态码格式：ω1ω2ω3vm1m2
    ...
    fold_angle = 45.0     # 折叠角度（内折用于抬起前磁铁）
    unfold_angle = 90.0   # 展开角度（外翻用于抬起后磁铁）
    
    # 生成轨迹和状态 - 使用蠕虫式爬行模式
    states_list = []
    
    # 初始状态：前足吸附，后足吸附，中心电机在基础角度
    # 初始状态：所有电机停止，前后磁铁吸附，中心电机在0度
    current_state = {
        'step': 0,
    ...
    while current_front[1] < target_front_z and step_count < max_steps:
        ...
        if step_count % 2 == 0:  # 移动前足
            # 状态变化：后足保持吸附，前足准备脱离
            # ω1ω2ω3vm1m2: 中心电机正转抬起前足，前磁铁脱附，后磁铁仍吸附
            detach_state = {
                'step': step_count + 1,
                'state_code': '001001',  # 中心电机正转(内折)抬起前足，前磁铁脱附，后磁铁仍吸附
                'front_magnet_status': '脱附',
                'rear_magnet_status': '吸附',
                'front_position': current_front.copy(),
                'rear_position': current_rear.copy(),
                'center_motor_angle': fold_angle,  # 内折抬起前磁铁（通过正转）
            ...
            if min(robot_params['d1'] + robot_params['d2'], new_spacing) <= robot_params['d1'] + robot_params['d2'] + robot_params['s_max']:
                ...
                # ω1ω2ω3vm1m2: 直线电机收缩(2=反转)，中心电机保持折叠状态
                fold_state = {
                    'step': step_count + 1,
                    'state_code': '001201',  # 直线电机收缩，中心电机保持正转状态抬起前足
                    'front_magnet_status': '脱附',
                    'rear_magnet_status': '吸附',
                    'front_position': new_front_pos,
                    'rear_position': current_rear.copy(),
                    'center_motor_angle': fold_angle,  # 保持前磁铁抬起状态
                ...
                prep_lower_state = {
                    'step': step_count + 1,
                    'state_code': '002001',  # 中心电机反转(外翻)准备放下前足，直线电机停止，前磁铁仍脱附，后磁铁吸附
                    'front_magnet_status': '脱附',
                    'rear_magnet_status': '吸附',
                    'front_position': new_front_pos,
                    'rear_position': current_rear.copy(),
                    'center_motor_angle': base_angle,  # 准备放下前磁铁（通过反转回基础角度）
                    'linear_motor_pos': abs(new_front_pos[1] - current_rear[1]) - (robot_params['d1'] + robot_params['d2']),
                    'front_magnet_attach_pos': new_front_pos,  # 即将吸附的位置
                    'rear_magnet_attach_pos': current_rear.copy()
                }
                states_list.append(prep_lower_state.copy())
                ...
                attach_state = {
                    ...
                    'front_position': new_front_pos,
                    'rear_position': current_rear.copy(),
                    'center_motor_angle': base_angle,  # 前磁铁吸附时回到基础角度
                    'linear_motor_pos': abs(new_front_pos[1] - current_rear[1]) - (robot_params['d1'] + robot_params['d2']),
                ...
                if min(robot_params['d1'] + robot_params['d2'], new_spacing) <= robot_params['d1'] + robot_params['d2'] + robot_params['s_max']:
                    ...
                    detach_state = {
                        'step': step_count + 1,
                        'state_code': '002010',  # 中心电机反转(外翻)抬起后足，后磁铁脱附，前磁铁仍吸附
                        'front_magnet_status': '吸附',
                        'rear_magnet_status': '脱附',
                        'front_position': current_front.copy(),
                        'rear_position': new_rear_pos,
                        'center_motor_angle': unfold_angle,  # 外翻抬起后磁铁（通过反转）
                    ...
                    extend_state = {
                        'step': step_count + 1,
                        'state_code': '002110',  # 直线电机伸展(1=正转)，中心电机保持反转状态抬起后足
                        'front_magnet_status': '吸附',
                        'rear_magnet_status': '脱附',
                        'front_position': current_front.copy(),
                        'rear_position': new_rear_pos,
                        'center_motor_angle': unfold_angle,  # 保持后磁铁抬起状态
                        'linear_motor_pos': abs(current_front[1] - new_rear_pos[1]) - (robot_params['d1'] + robot_params['d2']),
                        'front_magnet_attach_pos': current_front.copy(),
                        'rear_magnet_attach_pos': current_rear.copy()
                    }
                    ...
                    prep_lower_state = {
                        'step': step_count + 1,
                        'state_code': '001010',  # 中心电机正转(内折)准备放下后足，直线电机停止，前磁铁吸附，后磁铁仍脱附
                        'front_magnet_status': '吸附',
                        'rear_magnet_status': '脱附',
                        'front_position': current_front.copy(),
                        'rear_position': new_rear_pos,
                        'center_motor_angle': base_angle,  # 准备放下后磁铁（通过正转回基础角度）
                        'linear_motor_pos': abs(current_front[1] - new_rear_pos[1]) - (robot_params['d1'] + robot_params['d2']),
                        'front_magnet_attach_pos': current_front.copy(),
                    ...
                    attach_state = {
                        ...
                        'front_position': current_front.copy(),
                        'rear_position': new_rear_pos,
                        'center_motor_angle': base_angle,  # 后磁铁吸附时回到基础角度
                        'linear_motor_pos': abs(current_front[1] - new_rear_pos[1]) - (robot_params['d1'] + robot_params['d2']),
                        'front_magnet_attach_pos': current_front.copy(),
...
# 生成状态列表
states_list, base_angle, fold_angle, unfold_angle = generate_motor_positions_and_states_final()

# 写入详细状态表到文件
with open('robot_detailed_state_table_final.txt', 'w', encoding='utf-8') as f:
    f.write('"""机器人运动状态详细记录表（最终修正版 - 正确的中心电机角度控制）\n')
    f.write('时间：2026-01-28\n')
    f.write('任务：从初始位置(前足0,40；后足0,0)爬行至目标位置(前足0,1000)\n')
    f.write('基于优化轨迹（255步）\n')
    f.write('使用六位数字状态码：ω1ω2ω3vm1m2\n')
    f.write('正确实现：中心电机角度范围为0-90度，正转为内折，反转为外翻\n')
    f.write('"""\n\n')
...
print("机器人运动状态详细记录表（最终修正版）已生成完成")
...
print("\n中心电机角度控制：")
print("- 0度：基础角度，两臂共线")
print("- 45度：折叠角度（内折），用于抬起前磁铁")
print("- 90度：展开角度（外翻），用于抬起后磁铁")
print("- 正转(1)：内折（两上表面相互靠近）")
print("- 反转(2)：外翻（两上表面相互远离）")
print("\n表格格式说明：")
