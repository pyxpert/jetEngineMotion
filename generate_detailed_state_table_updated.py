...
def generate_motor_positions_and_states_updated():
    """
    生成机器人运动过程中的详细状态表（使用六位数字状态码）
    状态码格式：ω1ω2ω3vm1m2
...
# 生成状态列表
states_list = generate_motor_positions_and_states_updated()

# 写入详细状态表到文件
with open('robot_detailed_state_table_updated.txt', 'w', encoding='utf-8') as f:
    f.write('"""机器人运动状态详细记录表（更新版）\n')
    ...
    # 表头
    f.write(f"{'步数':<6} {'状态码':<10} {'前足磁铁':<10} {'后足磁铁':<10} {'前足位置':<15} {'后足位置':<15} {'中心电机角度':<12} {'直线电机位置':<12} {'前足吸附位置':<15} {'后足吸附位置':<15}\n")
    f.write("-" * 140 + "\n")
    ...
    for state in states_list:
        f.write(f"{state['step']:<6} {state['state_code']:<10} {state['front_magnet_status']:<10} {state['rear_magnet_status']:<10} "
                ...
                f"{state['linear_motor_pos']:<12.1f} {str(state['front_magnet_attach_pos']):<15} {str(state['rear_magnet_attach_pos']):<15}\n")

print("机器人运动状态详细记录表（更新版）已生成完成")
...
print("\n表格格式说明：")
print(f"{'步数':<6} {'状态码':<10} {'前足磁铁':<10} {'后足磁铁':<10} {'前足位置':<15} {'后足位置':<15} {'中心电机角度':<12} {'直线电机位置':<12} {'前足吸附位置':<15} {'后足吸附位置':<15}")
print("-" * 140)
