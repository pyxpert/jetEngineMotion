"""
机器人运动状态详细记录表（修正版 - 正确的电机动作逻辑和时间参数）
时间：2026-01-29
...
def generate_corrected_motor_positions_and_states_with_time():
    """
    ...
    while current_front[1] < target_front_z and step_count < max_steps:
        ...
        if step_count % 2 == 0:  # 移动前足
            ...
            if min(robot_params['d1'] + robot_params['d2'], new_spacing) <= robot_params['d1'] + robot_params['d2'] + robot_params['s_max']:
                ...
                # ω1ω2ω3vm1m2: 直线电机伸展(2=反转)，中间旋转电机保持抬起状态
                linear_move_state = {
                    'step': step_count + 1,
                    'state_code': '001201',  # 中间旋转电机保持抬起状态，直线电机伸展，前磁铁脱附，后磁铁吸附
                    'front_magnet_status': '脱附',
                    'rear_magnet_status': '吸附',
                    'front_position': new_front_pos,
                    'rear_position': current_rear.copy(),
                    'center_motor_angle': lift_angle,  # 保持抬起前磁铁
                    'linear_motor_pos': linear_motor_delta,
                    'front_magnet_attach_pos': current_front.copy(),  # 旧吸附位置
                    'rear_magnet_attach_pos': current_rear.copy(),
                    'duration_ms': int(abs(linear_motor_delta - (abs(current_front[1] - current_rear[1]) - (robot_params['d1'] + robot_params['d2']))) / motor_speeds['linear_speed'] * 1000)  # 直线电机运行时间
                }
                states_list.append(linear_move_state.copy())
                ...
                if min(robot_params['d1'] + robot_params['d2'], new_spacing) <= robot_params['d1'] + robot_params['d2'] + robot_params['s_max']:
                    ...
                    linear_move_state = {
                        'step': step_count + 1,
                        'state_code': '002110',  # 中间旋转电机保持抬起状态，直线电机收缩，前磁铁吸附，后磁铁脱附
                        'front_magnet_status': '吸附',
                        'rear_magnet_status': '脱附',
                        'front_position': current_front.copy(),
                        'rear_position': new_rear_pos,
                        'center_motor_angle': -lift_angle,  # 保持抬起后磁铁
                        'linear_motor_pos': linear_motor_delta,
                        'front_magnet_attach_pos': current_front.copy(),
                        'rear_magnet_attach_pos': current_rear.copy(),
                        'duration_ms': int(abs(linear_motor_delta - (abs(current_front[1] - new_rear_pos[1]) - (robot_params['d1'] + robot_params['d2']))) / motor_speeds['linear_speed'] * 1000)  # 直线电机运行时间
                    }
                    states_list.append(linear_move_state.copy())
            ...
            if min(robot_params['d1'] + robot_params['d2'], new_spacing) <= robot_params['d1'] + robot_params['d2'] + robot_params['s_max']:
                ...
                if min(robot_params['d1'] + robot_params['d2'], new_spacing) <= robot_params['d1'] + robot_params['d2'] + robot_params['s_max']:
                    ...
                    linear_move_state = {
                        ...
                        'front_magnet_attach_pos': current_front.copy(),  # 旧吸附位置
                        'rear_magnet_attach_pos': current_rear.copy(),
                        'duration_ms': int(abs(linear_motor_delta - (abs(new_front_pos[1] - current_rear[1]) - (robot_params['d1'] + robot_params['d2']))) / motor_speeds['linear_speed'] * 1000)  # 直线电机运行时间
                    }
                    states_list.append(linear_move_state.copy())
...
# 生成状态列表
states_list, base_angle, lift_angle = generate_corrected_motor_positions_and_states_with_time()

# 写入详细状态表到文件
with open('robot_detailed_state_table_corrected_with_time_and_speed.txt', 'w', encoding='utf-8') as f:
    f.write('"""机器人运动状态详细记录表（修正版 - 正确的电机动作逻辑和时间参数）\n')
...
print("机器人运动状态详细记录表（修正版 - 正确的电机动作逻辑和时间参数）已生成完成")
